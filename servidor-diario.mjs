...
const credencialApi = process.env.COGNITIVE_LEDGER_API_CREDENTIAL;
const apiUrl = process.env.COGNITIVE_LEDGER_API_URL;
const porta = Number(process.env.PORT || 10000);

if (!usuario || !senha || !credencialApi || !apiUrl) {
  console.error("Configuração obrigatória do Cognitive Ledger ausente.");
  process.exit(1);
}

try {
  const verificacao = await verificarApi({ usuario, senhaApi: credencialApi, apiUrl });
...
const servidor = criarServidor({ pastaPublica, usuario, senha, senhaApi: credencialApi, apiUrl });
