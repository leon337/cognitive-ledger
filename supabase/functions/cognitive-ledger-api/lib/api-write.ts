...
function validarEvento(valor: unknown): Record<string, unknown> {
  ...
  for (const chave of Object.keys(evento)) {
    if (!EVENT_KEYS.has(chave)) {
      throw new ErroMemoriaWrite("evento_campo_desconhecido");
    }
  }
  const id = texto(evento.id, "id", MAX_ID);
  if (!/^[A-Za-z0-9._:-]+$/u.test(id)) {
    throw new ErroMemoriaWrite("id_invalido");
  }
  const timestamp = texto(evento.timestamp, "timestamp", 128);
  if (!Number.isFinite(Date.parse(timestamp))) {
    throw new ErroMemoriaWrite("timestamp_invalido");
  }
  ...
  const status = evento.status === undefined
    ? "ativo"
    : texto(evento.status, "status", 128);
  const contexto = evento.contexto === undefined
    ? ""
    : String(evento.contexto).trim();
  if (contexto.length > MAX_TEXT) {
    throw new ErroMemoriaWrite("contexto_invalido");
  }
  ...
  if (evento.metadados !== undefined) {
    ...
    if (serializado.length > MAX_TEXT) {
      throw new ErroMemoriaWrite("metadados_muito_grandes");
    }
...
function listaObjetos(
  ...
  return valor.map((item) => {
    ...
    if (JSON.stringify(obj).length > MAX_TEXT) {
      throw new ErroMemoriaWrite(`${campo}_muito_grande`);
    }
    return obj;
  });
}
...
async function corpo(req: Request): Promise<Record<string, unknown>> {
  ...
  if (bruto.length > MAX_BODY) {
    throw new ErroMemoriaWrite("corpo_muito_grande", 413);
  }
  try {
  ...
  if (valor && typeof valor === "object") {
    ...
    return `{${
      Object.keys(obj).sort().map((k) =>
        `${JSON.stringify(k)}:${canonico(obj[k])}`
      ).join(",")
    }}`;
  }
...
function projecaoVerificacao(
  ...
  return Object.fromEntries(
    campos.filter((c) => c in evento).map((c) => [c, evento[c]]),
  );
}
...
export async function tratarRotaWrite(
  ...
  const status = await deps.repositorio.registrarEvento({
    evento,
    fontes,
    relacoes,
  });
  ...
  if (esperadoSha !== lidoSha) {
    throw new ErroMemoriaWrite("readback_divergente", 503);
  }
